...
for(s in Sites){
    ...
    otu_avg_cst = otu_avg_cst %>% group_by(OTU_ID, site) %>% mutate(max_value = var(value)) %>% group_by(site, variable) %>% mutate(site_rank = rank(-max_value)) 
otu_avg_cst_lst = otu_avg_cst
    otu_avg_cst = otu_avg_cst %>% filter(site == s, site_rank < 30) %>% mutate(OTU_ID = forcats::fct_reorder(factor(OTU_ID), .x = site_rank)) %>% group_by(variable, site) %>% arrange(desc(OTU_ID), .by_group = TRUE) %>% mutate(cum_sum = cumsum(value), lab_pos = lag(cum_sum), line_pos = cum_sum - value/2)
...
mat_rec = mat_cst %>% select(contains('REC')) #%>% scale_it
#rownames(mat_cst) = rownames(mat_nas) = otu_avg_cst_mat$OTU_ID
rownames(mat_nas) = rownames(mat_thr) = NULL
rownames(mat_rec) = make.unique(otu_avg_cst_mat$OTU_ID)
Heatmap(mat_nas) + Heatmap(mat_thr) + Heatmap(mat_rec)
```
    ...
    lblwidth = 0.2 * nrow(count), col = DirichletMultinomial:::.gradient) 
{
    ...
    taxa = intersect(rownames(pN), taxa_try)
...
for(site in c('nas', 'rec', 'thr')){
    ...
    heatmapdmn2(countm, adj_best, taxa = otu_avg_cst_mat$OrigName)
}
```
