...
for(s in Sites){
    otu_avg_cst = read_table2('DMN Cluster Selection/CST_Comp_Table_L6.txt', comment = '#') %>% melt() %>% mutate(site = str_extract(variable, 'NAS|REC|THR')) %>% left_join(print_labels %>% mutate(OTU_ID = str_replace_all(OrigName, fixed('|'), ';'))) %>% mutate(print_label = ifelse(is.na(`Printable Label`), OTU_ID, `Printable Label`))
...
Heatmap(mat_nas, col = ramp) + Heatmap(mat_thr, col = ramp) + Heatmap(mat_rec, col = ramp)
```

### Make heatmaps of CST

```{r heatmapdmn2, eval = FALSE}
...
pairs_signed_p = pairs_signed_p_all  %>% filter(Predictor %like% '(REC|NAS|THR)_CST_[0-9]+') %>%  mutate(Predictor = str_extract(Predictor, 'CST_[0-9]')) %>% group_by(pair1, otu) %>% mutate(min_pval_overall = min_na_nowarn(adj_pval, na.rm = TRUE))
...
```{r between_subj_assoc}
null_assoc = list.files('Composition Modeling/Null Models/', full.names = TRUE) %>% data_frame(fname = .) %>% mutate(site = str_extract(fname, 'NAS|REC|THR'), level = str_extract(fname, 'L[2-7]')) %>% rowwise() %>% mutate(assoc = list(read_csv(fname))) %>% unnest() %>% group_by(term) %>% mutate(adj_pval = p.adjust(p.value, method = 'fdr'))
...
ggplot(pairs_signed_month, aes(x = estimate, y = -log10(adj_pval), color = site)) + geom_point() + facet_wrap(~term, scales = 'free') + theme_minimal() + geom_hline(yintercept = 1, lty = 2) + ylab('-log10(FDR)') #+ geom_text(data = filter(pairs_signed_month, adj_pval < .1), aes(label = `Printable Label`))
