...
for(s in Sites){
    otu_avg_cst = read_table2('DMN Cluster Selection/CST_Comp_Table_L7.txt', comment = '#') %>% melt() %>% mutate(site = str_extract(variable, 'NAS|REC|THR')) %>% left_join(print_labels %>% mutate(OTU_ID = str_replace_all(OrigName, fixed('|'), ';'))) %>% rename('Printable Label' = 'print_label')
...
for(site in c('nas', 'thr', 'rec')){
    ...
    heatmapdmn2(countm, adj_best, taxa = otu_avg_cst_mat$OrigName, col = viridis::magma(10))
}
```
...
pairs_signed_month = filter(pairs_signed_p_all, Predictor %in% c('ModeOfDeliveryVaginal_Breech', 'ModeOfDeliveryVaginal_Vertex', 'Birth.SeasonJan-Mar',         'Birth.SeasonJuly-Sep', 'Birth.SeasonOct-Dec', 'gaBirth'))

ggplot(pairs_signed_month, aes(x = beta, y = -log10(adj_pval), color = pair1)) + geom_point() + facet_wrap(~Predictor, scales = 'free') + theme_minimal() + geom_hline(yintercept = 1, lty = 2)

filter(pairs_signed_month, adj_pval < .2)
```
