...
biom_files = data.table(fname = list.files(path = 'data', pattern = '.*_L[2-7].txt', full.names = TRUE))
...
Use ? normalized files.

```{r load}
bioms = lapply(biom_files[taxa=='L7', fname], function(x) read_tsv(x) %>% rename(SampleID = 'Sample.Name'))
names(bioms) = biom_files[taxa=='L7', site]
avail_sample = melt(plyr::llply(bioms, '[[', 'Sample.Name')) %>% select(`Sample.Name` = value, `Primary.Type` = L1) %>% data.table()
...
generate_pairing = function(set, min_nz = 10, min_pct = .05){
    ...
    mat_set = list()
    for(m in set){
        cid = nas_rec[`Primary.Type` == m, list(Sample.Name)]
        assay = semi_join(bioms[[m]], cid, by = 'Sample.Name', copy = TRUE) %>% dplyr::select(-Sample.Name) %>% as.matrix() %>% t()
        ...
        mat_set[[m]] = SummarizedExperiment(assays=asin_sqrt(assay), colData=semi_join(nas_rec, cid, by = 'Sample.Name'), rowData=data.table(taxa=rd, sname=sname, original_name=oname))
...
ggplot(stats_all, aes(x = value, fill = site)) + geom_density(alpha = .5) + facet_wrap(~variable, scale='free')
...
pairs_matched_otu = purrr::map(pairs, function(p){
  common = intersect(rowData(p[[1]])$original_name, rowData(p[[2]])$original_name)
  map(p, ~ .x[rowData(.x)$original_name %in% common,])
})

otu_cor = purrr::map(pairs_matched_otu, function(p){
    x = getbiom(p[[1]], frequency = FALSE) %>% fit_lm(data_ = colData(p[[1]]), formula_ = ~ splines::ns(DOL + CGA, 10) + Participant.ID) %>% resid_lm()
    y = getbiom(p[[2]], frequency = FALSE) %>% fit_lm(data_ = colData(p[[2]]), formula_ = ~ splines::ns(DOL + CGA, 10) + Participant.ID) %>% resid_lm()
    
    dc = diag(cor(x, y))
    data_frame(cor = dc, otu = colnames(x))
})
...
cv_dof = dof_df %>% group_by_all() %>% do(cv_coord = {
    ...
    crossval_cc(pairs[[.$pair]][[1]], pairs[[.$pair]][[2]], formula_ = formula_, center = FALSE, nfolds = 5)
})

pairs_id_df = as_data_frame(t(pairs_ids)) %>% dplyr::rename(site1 = V1, site2 = V2)  %>% rowid_to_column(var = 'pair')
cv_dof_flat = cv_dof  %>%unnest() %>% filter(coordinate < 10, metric == 'cor') %>% left_join(pairs_id_df)


ggplot(cv_dof_flat, aes(x = coordinate, y = mean, color = sample, fill = sample))+geom_point() + geom_ribbon(aes(ymin = mean - 2*sd, ymax = mean + 2*sd), alpha = .4) + theme_minimal() + facet_grid(Dof ~ site1+site2) + geom_hline(yintercept = 0, lty = 2) + scale_x_continuous(breaks = c(1, 5, 10)) + ylab("Canonical corr. coef.") + theme(legend.position = 'bottom') + coord_cartesian(ylim=c(-.2, 1)) + scale_y_continuous(breaks = c(0, .5, 1))
...
1. For NAS/REC, NAS/THR, REC/THR
2. Find reduced model
3. Do CCA CV (and pick number of coordinates)
4. Show canonical coordinates

```{r full_cc}
full_dof = cv_dof = dof_df %>% group_by_all() %>% do(cv_coord = {
    env = environment()
    env$dof = .$dof
    ## for some reason, spline can't find DOF unless we do this.  something about forcing evaluation.
    formula_ = if(.$dof>1){
        as.formula(sprintf('~splines::ns(DOL, %s)', .$dof))}
    else if(.$dof==1){
        formula(~DOL)
        } else{
            formula(~1)
        }
    do_cc(pairs[[.$pair]][[1]], pairs[[.$pair]][[2]], formula_ = formula_, center = FALSE)
})
...
```{r cc_loadings, fig.width = 12, fig.height = 8}
cc_scores = full_dof %>% rowwise() %>% mutate(scores = {
    mx = melt(cv_coord$xcoef[,1:3], variable.factor = FALSE) %>% mutate(var = 'x')
    my = melt(cv_coord$ycoef[,1:3], variable.factor = FALSE)  %>% mutate(var = 'y')
    list(rbind(mx, my))
}) %>% select(-cv_coord) %>% unnest()
max_val = cc_scores %>% group_by(dof, pair, Var1, var) %>% summarize(max_val = max(abs(value))) %>% group_by(dof, pair, var) %>% mutate(rank_max = rank(-max_val)) %>% filter(rank_max < 20)

cc_scores = left_join(max_val, cc_scores) %>% filter(dof == 14)



ggplot(cc_scores, aes(x=reorder(Var1, value, min), y=value, color=var, label = Var2)) + geom_text() + facet_wrap(~pair, scales='free') + coord_flip() + theme_minimal()
