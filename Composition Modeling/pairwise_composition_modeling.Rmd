...
constCols <- c("gaBirth", "ModeOfDelivery", "ns(DOL, 14)")
...
workDir <- "./"##~/Desktop/MiSeq_Projects/12.00012_Microbiome_Master/Substudy_2_1a/analysis/stage_analysis_workspace/rigor_analysis/composition_modeling_data/workspace" #Directory where files will be read from and written to
minSampFrac <- 0.05 #Minimum fraction of samples in which an OTUs abundance must be non-zero in order to be included

andrew_rsq = function(empty, null, full){
    (deviance(full) - deviance(null))/deviance(empty)
}
...
for (site in bodySites) {
  ...
  for (otherSite in otherSites) {
    ...
    for (tax in taxLevels) {
      ...
      raw_otu_table <- read_tsv(paste(workDir, sprintf("%s_%s.txt", site, tax), sep = "/"))
      colnames(raw_otu_table) <- make.names(colnames(raw_otu_table), unique = TRUE)
      mapping <- read_tsv(paste(workDir, sprintf("%s_Mapping.txt", site), sep = "/"))
      ...
      for(otu_i in 1:num_otus) {
        ...
        #rsq[otu_i] = andrew_rsq(empty_fits[[otu_i]], null_fits[[otu_i]], full_fits[[otu_i]])
        rsq[otu_i] = nakagawa_rsq(empty_fits[[otu_i]], null_fits[[otu_i]], full_fits[[otu_i]])
         #browser(expr=otu_ids[otu_i] == 'k__Bacteria.p__Actinobacteria.c__Actinobacteria.o__Actinomycetales.f__Corynebacteriaceae.g__Corynebacterium.s__')
      }
